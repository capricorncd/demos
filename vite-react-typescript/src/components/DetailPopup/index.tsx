...
export default function DetailPopup(props: DetailProps) {
  ...
  const specificationCategories = useSelector<RootState>(
    (state) => state.data.specificationCategories,
  ) as StoreDataSpecCategories
  ...
  const price = selectedItem && selectedItem.count ? selectedItem.count * (data.price + specPrice) : 0
    ...
    <section className={classes.join(' ')}>
      <section className="common-popup__inner shadow">
        ...
        <div className="body" ref={popupBodyRef}>
          ...
          {Object.values(specificationData).map((item, index) => (
            <div key={index}>
              <AppTitle>
                {item.name}
                <span className="small-text">{item.is_multiple_choice ? '（多数選択可能）' : null}</span>
              </AppTitle>
              {item.list.map((v, k) => (
                <CheckBox
                  key={k}
                  data={v}
                  checked={specifications.some((item) => item.id === v.id)}
                  change={handleSelect}
                />
              ))}
            </div>
        ...
        <div className="footer">
          {price ? (
            <AppPrice className={`total-price`} fontSize={24} primary>
              {price}
            </AppPrice>
          ) : null}
          <div className="plus-items gray">{specifications.map((item) => item.name).join('、')}</div>
          <AppButton className={`btn-submit`} onClick={handleClose}>
            确定
          </AppButton>
